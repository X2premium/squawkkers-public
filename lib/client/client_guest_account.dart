...
class TwitterGuestAccount {
  ...
  static Future<String> _getWelcomeFlowToken(
    Map<String, String> headers,
    String accessToken,
    String guestToken,
  ) async {
    log.info(
      'Posting https://api.twitter.com/1.1/onboarding/task.json?flow_name=welcome',
    );
    headers.addAll({
  ...
  static Future<TwitterTokenEntity> _getGuestTwitterTokenFromTwitter(
    ...
    if (response.statusCode == 200 && response.body.isNotEmpty) {
      ...
      if (subtasks != null) {
        var accountElm = subtasks.firstWhereOrNull(
          (task) => task['subtask_id'] == 'OpenAccount',
        );
        if (accountElm != null) {
          var account = accountElm['open_account'];
          log.info('Guest Twitter/X token created.');
  ...
  static Future<TwitterTokenEntity> createGuestTwitterToken() async {
    ...
    String flowToken = await _getWelcomeFlowToken(
      headers,
      accessToken,
      guestToken,
    );
    TwitterTokenEntity guestTwitterToken =
        await _getGuestTwitterTokenFromTwitter(headers, flowToken);
